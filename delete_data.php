<?php
	...
	$image_detail = $wpdb->get_results(
		...
		$wpdb->prepare(
			"SELECT image_order,image_pos from `$table_name` WHERE `$table_name`.`image_id` = %d",
			$image_id
	...
	$all_pos = $wpdb->get_results(
		...
		// takes 2 parameter one query and one string
		$wpdb->prepare(
			"SELECT * from `$table_name` WHERE `$table_name`.`image_pos` > %d",
			$image_detail[0]['image_pos']
			// by ARRAY_A we convert std objet to array object
		),
		ARRAY_A
	);
	...
	$all_order = $wpdb->get_results(
		...
		// takes 2 parameter one query and one string
		$wpdb->prepare(
			"SELECT * from `$table_name` WHERE `$table_name`.`image_order` > %d",
			$image_detail[0]['image_order']
			// by ARRAY_A we convert std objet to array object
		),
		ARRAY_A
	);
